...
def sinterplotthreshold(ax,mylist,order,rot,mem,num_rounds,find_pL_per,mind=0,maxd=100,maxp=1,minp=0.001,plot_values_near_005 = False):
...
def sinterplotthreshold_same_start_colour(ax,mylist,order,rot,mem,num_rounds,find_pL_per,mind=0,maxd=100,maxp=1,minp=0.001,plot_values_near_005 = False):
...
def plot_thresholds(mylist, orders, mind = 2, minp = 0, maxp = 1, rotations = [['unro'], ['ro']],  output_dir='plots/thresholds',maxds = [10000], find_pL_per = 'd rounds'):

    # choices for find_pL_per are 'round', 'd rounds' or the entire 'shot'
    orders = [orders]
    ...
    for rots in rotations:
        ...
        for mem in 'xz':
            ...
            for rot, order, maxd in zip(rots, orders, maxds):
                sinterplotthreshold_same_start_colour(
                    ax, mylist, order, rot, mem, '3d', find_pL_per, mind, maxd, maxp, minp
                )
                if rot == 'unro':
        ...
        ax.set_title(f'Rotated') if rots[0] == 'ro' else ax.set_title(f'Unrotated')
        

        fig.savefig(f'{output_dir}/{rots[0]}{str(orders[0])}_threshold_plot.pdf', format='pdf')
...
def sinterplotpLvQ_overlaid(b,noise,mind,maxd,unroorder,roorder,mylist,num_rounds,find_pL_per,ax,sizeoffont,noise_model,rotation,ps,markers,colours,pltqubits = False):
    ...
    sinter.plot_error_rate(   
        ...
        and
        # (s.json_metadata['p'] > 0.001 if (s.json_metadata['d']>=16 and s.json_metadata['ro']=='unro') else s.json_metadata['p']>0) 
...
def plot_pL_vs_qubit_count(mylist, b, roorder = 10231203, unroorder = 10231203, ps = None, ro_min_d = 2, unro_min_d = 2,  xlims = [4,20], ylims = [1e-6, None], noise_model = 'SD'):
    ...
    for rotation in ['ro','unro']:
        if rotation == 'ro':
            minds = [ro_min_d for _ in ps]
            # minds = [8 for _ in ps]
            maxds = [17,22] +  [22 for _ in range(len(ps)-2)]
        else:
            minds = [unro_min_d for _ in ps]
            # minds = [6 for _ in ps]
        ...
        for noise, mind, maxd in zip(ps, minds, maxds):
            ...
            sinterplotpLvQ_overlaid(b, noise, mind, maxd, unroorder, roorder, mylist, num_rounds, find_pL_per, ax, fontsize, noise_model, rotation,ps,markers,colours,pltqubits)
...
def fit_scaling_and_plot(combined_list, distances, basis, roorder, unroorder, ro_min_d = 2, ro_max_d = 1000, unro_min_d = 2, unro_max_d = 1000, uncertainty_weighting_option = 2, optional_plot = True):

    # In paper used ro_min_d = 8, unro_min_d = 6, (distances below this didn't fit scaling)
    # ro_max_d = 22, unro_max_d = 17  (didn't have enough data for distances above these)
    ...
    for rot, order, mem in zip(['unro','ro'],[roorder,unroorder],[basis,basis]):


        mind = ro_min_d if rot == 'ro' else unro_min_d
        maxd = ro_max_d if rot == 'ro' else unro_max_d
        ...
        for d in ds: # find m and b for each distance's log10(pL) = m * log10(p) + b 
            ...
            if len(this_ds_ps) == 0:
                ...
                sys.exit(f"Line {line_number + 1}: no circuits matched b = {mem}, rt = {rot} , o = {order} in input csv file.")
                break
            ...
            if optional_plot:
                ...
                scatterplot = plt.scatter(this_ds_ps, this_ds_pLs, marker = '.', label = f"d = {d}", color = color)
                ...
                plt.fill_between(this_ds_ps, upper_log_pL, lower_log_pL, color = color, alpha = 0.3, zorder=1)
                index += 1
        ...
        if optional_plot:
            ...
            for j in range(len(these_ds)): # for d in list(range(6,30)): # if want to extrapolate
                ...
                linestyle = '--'
                plt.plot(ps, pL(ps, d), linestyle = linestyle, linewidth = 1, color = 'grey', label = 'Curve fit' if j == 0 else None)
...
def plot_teraquop(mylist, b, roorder, unroorder, ps = None,  noise_model = 'SD', ro_min_d = 2, unro_min_d = 2, optional_plot = False, teraquop_inset = False):
...
def extract_data_for_ratios(mylist, b, ps, roorder, unroorder, ro_min_d = 2, unro_min_d = 2, noise_model = 'SD', find_pL_per = 'd rounds',):
    ...
    colors = ['b', 'r', 'g', 'salmon', 'k', 'orange', 'purple']
...
def plot_ratio(mylist, b, roorder, unroorder, ps = None):
    ...
    data = extract_data_for_ratios(mylist, b, ps, roorder, unroorder)
    ...
    for i in reversed(range(len(ps))):  # can do len(ps) -1 here to not plot p = 0.0055 
        ...
        plt.plot(x_solid, y_solid, color=colors[i], linestyle='-', label=f'p = {ps[i]}', linewidth = 2)
        
        # dashed lines for ratios calculated from line-fit projections:
        plt.plot(x_dash, y_dash, color=colors[i], linestyle='--', linewidth = 2)

        # X marks the limit as pL -> 0 
        plt.scatter(minpL, limit, color=colors[i], marker='x', s = 100, zorder = 3)

        plt.fill_between(x, y + Ey, y - Ey , alpha = 0.3, color = colors[i]) # uncertainty
...
def plot_memory_times(mylist, b, roorder, unroorder, ps = None, ro_min_d = 2, unro_min_d = 2):
    ...
    # ps = [0.0005, 0.001, 0.002, 0.003]
    colours = ['b', 'r', 'magenta', 'salmon', 'k', 'orange', 'purple','b', 'r', 'magenta', 'salmon', 'k', 'orange', 'purple']
    ...
    unromaxds = [13, 17, 17, 17, 17, 17, 17, 17]
